...
const getRestoByCuisine = async (req, res) => {
  ...
  try {
    const cuisineResto =
      await restoMethods.readRestaurantsByCuisine(cuisineName);
...
const updateResto = async (req, res) => {
  ...
  try {
    const updatedInfo = await restoMethods.updateRestaurant(
      restoId,
      updateData,
    );
...
const getRestoByLocation = async (req, res) => {
  ...
  try {
    const locationRestaurant =
      await restoMethods.searchRestaurantsByLocation(restoLocation);
...
const filterRestoByRating = async (req, res) => {
  ...
  try {
    const filteredResto =
      await restoMethods.filterRestaurantsByRating(ratingValue);
...
const deleteADishFromMenu = async (req, res) => {
  ...
  try {
    const deletedDish = await restoMethods.removeDishFromMenu(
      restoId,
      dishName,
    );
...
const addRestoReviews = async (req, res) => {
  ...
  try {
    const reviewResult = await restoMethods.addRestaurantReviewAndRating(
      restoId,
      reviewObj,
    );
...
const getAllRestoReviews = async (req, res) => {
  ...
  try {
    const reviewsResult =
      await restoMethods.getUserReviewsForRestaurant(restoId);
